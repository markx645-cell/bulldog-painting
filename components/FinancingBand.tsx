    ...
    <section className="section bg-pine text-white">
      <div className="container-x">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-slate-200">{financing.lead}</p>
        </div>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4" data-reveal-stagger>
          {financing.points.map((p) => (
            <div key={p.big} className="rounded-xl border border-white/20 bg-white/5 p-6 text-center">
              <span className="block font-display text-3xl font-extrabold text-brass-200">{p.big}</span>
              <span className="mt-2 block text-sm leading-snug text-slate-200">{p.small}</span>
            </div>
          ))}
        </div>

        <div className="mt-10 text-center">
          <Link href="/financing" className="btn-brass">
